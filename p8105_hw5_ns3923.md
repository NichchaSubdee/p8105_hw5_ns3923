...
**Explanation:** The trend is upward as the group size increases,
meaning that larger groups have a higher probability of two people
sharing a birthday. The probability rises and approaches 1 as the group
becomes larger. Around 23 people, the chance of a shared birthday is
about 50%.
...
**Explanation:** As the true mean increases, the power of the test also
...
![](p8105_hw5_ns3923_files/figure-gfm/P2_4-1.png)<!-- -->
...
(only when rejecting the null) look similar at higher true means, their
behavior is different when the true mean is small. The red line is close
to the true mean across all values of μ because the sample mean is an
unbiased estimator. However, the blue line is consistently higher than
the red line when μ is small. This happens because we are only looking
at samples where the null hypothesis is rejected, and those samples tend
to have larger estimated means. As a result, the average of the sample
means in the rejected group overestimates the true mean.
...
The `homicide-data.csv` is the dataset from the Washington Post which
has gathered data on homicides in 50 large U.S. cities. There are 12
variables and 52,179 homicide cases in this raw dataset. Some of key
variables are `city`,`state`,`victim_sex`, and `disposition` (case
status).

**2) Create a `city_state` variable and summarize numbers of homicides
and unsolved cases within cities**
...
```
    ...
    ## # A tibble: 51 × 4
    ##    city_state      total unsolved solved
    ##    <chr>           <int>    <int>  <int>
...
After creating the new variable `city_state`, there are 51 different
large cities in the dataset, including 50 large cities in different
states and the federal district of Washington, DC.

**3) For Baltimore, MD, use the prop.test function to estimate the
proportion of homicides that are unsolved, and estimate proportion and
...
**Answer:** The estimated proportion of homicides in Baltimore, MD that
are unsolved is 64.6% (95% CI: 62.8% - 66.3%)
...
**5) Create a plot that shows the estimates and CIs for each city, and
...
![](p8105_hw5_ns3923_files/figure-gfm/P3_plot-1.png)<!-- -->

**Explanation:** The plot illustrates that the proportion of unsolved
homicides varies a lot across cities. Cities like *Chicago, IL*, *New
Orleans, LA*, and *Baltimore, MD* have higher proportions of unsolved
cases, while cities such as *Richmond, VA* and *Charlotte, NC* have much
lower proportions. Cities with more homicide cases tend to have narrower
confidence intervals because the estimates are more stable. In contrast,
cities with only a few cases have wider confidence intervals due to
greater uncertainty. For example, *Tulsa, AL* has an estimated
proportion of 0 unsolved cases, but its confidence interval is very wide
because the number of total homicides is small.
